    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-white text-black p-6">
      ...
      {lastScore > 0 && (
        <div className="bg-white/20 rounded-lg p-4 mb-6 backdrop-blur-sm">
          <p className="text-sm opacity-80 font-bold">{t('Last score:')} {lastScore}</p>
        </div>
      )}

      <button
        onClick={onStartGame}
        className="w-full min-h-[50px] py-5 px-10 bg-blue-600 text-white text-3xl font-bold rounded-full shadow-lg hover:bg-blue-700 transition-colors"
      >
        Start Game
      </button>
